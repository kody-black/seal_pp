...
#include "seal-decl.h"
const int ifstmtvalue = 1;
const int whilestmtvalue = 2;
const int forstmtvalue = 3;
const int returnstmtvalue = 4;
const int continuestmtvalue = 5;
const int breakstmtvalue = 6;
const int stmtBlockstmtvalue = 7;
...
public:
	StmtBlock_class(VariableDecls a1, Stmts a2) {
		vars = a1;
	    stmts = a2;
		stmttypevalue = stmtBlockstmtvalue;
	}
...
public:
	ContinueStmt_class() {
	stmttypevalue = continuestmtvalue;
	}
    Stmt copy_Stmt();
	void check(Symbol);
    void dump_with_types(ostream&,int);
